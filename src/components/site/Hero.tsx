...
import { transformHero } from "@/services/transformData";
import { fbHero } from "@/data/fallbacks";
import { LogoMark } from "./Navbar";

export function Hero() {
  const { data } = useSheet("hero", transformHero, fbHero);
      ...
      <div className="container-prose relative grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
        <div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.25 }}
            ...
            <a href={data.ctaPrimaryHref} className="group inline-flex items-center gap-2 rounded-xl bg-foreground px-5 py-3 text-sm font-semibold text-background shadow-elev transition-transform hover:-translate-y-0.5">
              {data.ctaPrimary}
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </a>
            <a href={data.ctaSecondaryHref} className="inline-flex items-center gap-2 rounded-xl border border-border bg-white px-5 py-3 text-sm font-semibold text-foreground shadow-soft transition-colors hover:bg-secondary">
              {data.ctaSecondary}
            </a>
          </motion.div>

          <p className="mt-8 max-w-md text-sm text-muted-foreground">{data.intro}</p>
        </div>
...
import { useSheet as _u } from "@/hooks/useSheet";
import { transformClients } from "@/services/transformData";
import { fbClients } from "@/data/fallbacks";
function ClientStrip() {
  const { data } = _u("clients", transformClients, fbClients);
        ...
        >
          {duplicatedData.map((c, i) => (
            <div key={i} className="flex flex-none items-center justify-center text-muted-foreground/80">
              {c.logo ? (
                <img src={c.logo} alt={c.name} className="h-7 w-auto opacity-60 transition hover:opacity-100" loading="lazy" />
              ) : (
